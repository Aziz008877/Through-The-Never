...
public abstract class BaseEnemyMove : MonoBehaviour, IFrostbiteReceivable
{
    [Header("General")]
    ...
    [SerializeField] private int _frostStacks;
    [SerializeField] private int _frostMaxStacks;
    ...
    protected virtual void Update()
    {
        ...
        switch (_behaviour)
        {
            case MoveBehaviour.Idle:        IdleLogic(); break;
            case MoveBehaviour.Patrol:      PatrolLogic(); break;
            case MoveBehaviour.ChaseAlways: ChaseLogic(); break;
            case MoveBehaviour.ChaseIfInRange:
                if (_target != null && InChaseRange()) ChaseLogic();
                else PatrolLogic();
                break;
        }
    ...
    {
        if (_patrolPoints == null || _patrolPoints.Length == 0) { IdleLogic(); return; }

        if (_agent.remainingDistance <= _agent.stoppingDistance)
        {
            _pauseTimer += Time.deltaTime;
            if (_pauseTimer < _wanderPause) { StopMoving(); return; }
            _pauseTimer = 0f;
    ...
    {
        if (_target == null) { IdleLogic(); return; }

        _enemyAttack.SetTarget(_target);

        if (_enemyAttack.ShouldStopMovement()) { StopMoving(); return; }

        float sqrDist = (transform.position - _target.position).sqrMagnitude;
        float stopSqr = _stopDistance * _stopDistance;

        if (sqrDist <= stopSqr) StopMoving();
        else MoveTo(_target.position);
    }
    ...
    {
        Vector3 lookDir;
        if (_isMoving && _agent.velocity.sqrMagnitude > 0.01f) lookDir = _agent.velocity;
        else if (_target != null && !_isMoving) lookDir = _target.position - transform.position;
        else return;
    ...
    void FrostTick()
    {
        if (_frostTimeLeft > 0f)
        {
            _frostTimeLeft -= Time.deltaTime;
            if (_frostTimeLeft <= 0f) { _frostTimeLeft = 0f; _frostStacks = 0; }
        }
    }

    public void ApplyFrostbite(float slowPerStack, float dmgRedPerStack, float duration, int maxStacks)
    {
        _frostSlowPerStack = slowPerStack;
        _frostDmgRedPerStack = dmgRedPerStack;
        _frostMaxStacks = maxStacks;
        _frostTimeLeft = duration;
        _frostStacks = Mathf.Min(_frostStacks + 1, _frostMaxStacks);
    }

    public int FrostStacks => _frostStacks;
    public bool IsFrostActive => _frostStacks > 0 && _frostTimeLeft > 0f;
    public float MoveSpeedMul => Mathf.Clamp(1f - _frostStacks * _frostSlowPerStack, 0.2f, 1f);
    public float OutgoingDamageMul => Mathf.Clamp(1f - _frostStacks * _frostDmgRedPerStack, 0.2f, 1f);

    protected enum MoveBehaviour { Idle, Patrol, ChaseAlways, ChaseIfInRange }
}
