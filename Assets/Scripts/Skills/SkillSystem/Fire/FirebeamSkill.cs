...
public class FirebeamSkill : ActiveSkillBehaviour
{
    ...
    private void Update()
    {
        ...
        IDamageable target = FindClosest(Definition.Range);
        ...
        Vector3 end   = start + transform.forward * Definition.Range;

        if (Physics.Raycast(start, transform.forward, out var hit, Definition.Range))
        {
            end = hit.point;
        ...
        _beamVfx.transform.localScale = new Vector3(s.x, s.y, Vector3.Distance(start, end));
        ...
        if (_tickTimer >= _tickRate)
        {
            ...
            if (hit.collider != null && hit.collider.TryGetComponent(out IDamageable hitDmg))
            {
                float t   = _timer / Definition.Duration;
                ...
                hitDmg.ReceiveDamage(dmg, type);
            }
        }
        
        _timer += Time.deltaTime;
        if (_timer >= Definition.Duration) StopBeam();
    }
